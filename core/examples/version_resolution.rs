...
use polkadot_cookbook_core::constants::RECIPES_DIR;
use polkadot_cookbook_core::version::{
    load_global_versions, resolve_recipe_versions, VersionSource,
};
use std::path::Path;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Example 2: Resolve versions for a specific recipe
    let recipes_dir = repo_root.join(RECIPES_DIR);
    if recipes_dir.exists() {
        if let Ok(mut entries) = tokio::fs::read_dir(&recipes_dir).await {
            if let Some(entry) = entries.next_entry().await? {
                ...
                match resolve_recipe_versions(repo_root, &tutorial_slug).await {
                    Ok(resolved) => {
                ...
                note(
                    "Note",
                    format!("No recipes found in {}", recipes_dir.display()),
                )?;
            }
        }
    } else {
        note(
            "Note",
            format!("Recipes directory not found: {}", recipes_dir.display()),
        )?;
    }
