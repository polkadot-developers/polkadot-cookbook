...
use crate::version::types::{GlobalVersionConfig, RecipeVersionConfig};
use std::path::Path;
use tokio::fs;
...
impl VersionLoader {
    ...
    /// Load recipe-specific version configuration
    ///
    /// # Arguments
    /// * `path` - Path to the recipe's versions.yml file
    ///
    /// # Example
    /// ```no_run
    /// use polkadot_cookbook_core::version::VersionLoader;
    ...
    /// # }
    /// ```
    pub async fn load_recipe(path: &Path) -> Result<RecipeVersionConfig> {
        if !path.exists() {
            return Ok(RecipeVersionConfig {
        ...
        let contents =
            fs::read_to_string(path)
                .await
                .map_err(|e| CookbookError::FileSystemError {
                    message: format!("Failed to read recipe versions file: {e}"),
                    path: Some(path.to_path_buf()),
                })?;

        Self::parse_recipe(&contents)
    }
    ...
    /// Parse recipe version configuration from YAML string
    fn parse_recipe(yaml: &str) -> Result<RecipeVersionConfig> {
        serde_yaml::from_str(yaml).map_err(|e| {
            CookbookError::ConfigError(format!("Failed to parse recipe versions YAML: {e}"))
        })
    }
}
...
"#;
        ...
        let config = VersionLoader::parse_recipe(yaml).unwrap();
        assert_eq!(
            config.versions.get("polkadot_omni_node"),
...
"#;
    ...
    #[tokio::test]
    async fn test_load_nonexistent_recipe_returns_empty() {
        let result = VersionLoader::load_recipe(Path::new("/nonexistent/versions.yml")).await;
        assert!(result.is_ok());
