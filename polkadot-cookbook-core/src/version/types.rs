...
/// Tutorial-specific version configuration
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct TutorialVersionConfig {
...
impl TutorialVersionConfig {
    /// Create a new tutorial version config with default metadata
...
/// Resolved version configuration after merging global and tutorial-specific versions
#[derive(Debug, Clone, PartialEq)]
pub struct ResolvedVersions {
    /// The merged version set (global + tutorial overrides)
    pub versions: VersionSet,
...
pub enum VersionSource {
    ...
    Global,
    /// Version came from tutorial-specific versions.yml
    Tutorial,
}
...
mod tests {
    ...
    #[test]
    fn test_tutorial_version_config_new() {
        let mut versions = HashMap::new();
        versions.insert("rust".to_string(), "1.86".to_string());

        let config = TutorialVersionConfig::new(versions.clone());
